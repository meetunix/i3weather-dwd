...
def write_file(vals, file_path):
    s = "{} T: {}  H: {}  W: {} ({})".format(
        vals["time"], vals["temp"], vals["humidity"], vals["maxwind"], vals["weather"]
    )
...
def parse_args():
    ...
    config = {"station_file": None, "pid": None, "daemon": False, "german": False}

    parser = argparse.ArgumentParser()

    parser.add_argument(
        "-s", "--station", required=True, help="id of the weather station (mandatory)"
    )
    parser.add_argument(
        "-p", "--pid", help="path to the pid file (default {})".format(PID_DEFAULT)
    )
    parser.add_argument(
        "-d", "--daemon", action="store_true", help="run as daemon (EXPERIMENTAL)"
    )
    parser.add_argument(
        "-g",
        "--german",
